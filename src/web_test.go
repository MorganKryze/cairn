...
func storeModel(t *testing.T, files map[string]string) *Config {
	t.Helper()
	cfg, err := loadConfig(writeFiles(t, files))
	if err != nil {
		t.Fatal(err)
	}
...
func TestConfigHelpers(t *testing.T) {
	cfg := testConfig(t)
	if n := countServices(cfg); n != 2 {
		t.Errorf("countServices = %d, want 2", n)
	}
