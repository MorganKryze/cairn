...
func TestCategoryTrail(t *testing.T) {
	m, err := BuildModel(sample(t), nil) // two categories: documents + other
	if err != nil {
		t.Fatal(err)
	}
...
func TestSitePagesAndQuickLinks(t *testing.T) {
	...
	if _, err := config.Load(dir); err == nil || !strings.Contains(err.Error(), "collides") {
...
func TestAboutAndLinkIcons(t *testing.T) {
	...
	if _, err := config.Load(dir); err == nil || !strings.Contains(err.Error(), "built-in glyph") {
...
func TestCanonicalAndHreflang(t *testing.T) {
	...
	plain, err := config.Load(testutil.WriteFiles(t, map[string]string{
		"services.yaml": "- {id: a, url: https://a.example.org, name: A}\n",
	}))
	if err != nil {
		t.Fatal(err)
...
func TestUnknownServiceKeyIsAnError(t *testing.T) {
	...
	if _, err := config.Load(dir); err == nil || !strings.Contains(err.Error(), "descr") {
...
func TestPageSections(t *testing.T) {
	...
	if _, err := config.Load(dir); err == nil || !strings.Contains(err.Error(), "body or sections") {
