...
| -------------------- | -------- | -------------------------------------------------------------- |
| `cairn-1.19.3.tgz`   | 4 KB     | `helm pull`, the signed artifact rather than the folder in git |
...
```sh
just save 1.19.3 linux/amd64   # the platform of the cluster, not of your laptop
```
...
```sh
helm pull oci://ghcr.io/morgankryze/charts/cairn --version 1.19.3

cosign verify ghcr.io/morgankryze/charts/cairn:1.19.3 \
  --certificate-identity-regexp '^https://github.com/MorganKryze/cairn/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```

The same two lines for `morgankryze/cairn:1.19.3`, the image. Keep the
...
docker pull --platform $PLATFORM morgankryze/cairn:1.19.3
...
```sh
docker tag morgankryze/cairn:1.19.3 harbor.internal/cairn:1.19.3
docker tag ghcr.io/twin/gatus:v5.36.0 harbor.internal/gatus:5.36.0
docker save harbor.internal/cairn:1.19.3 harbor.internal/gatus:5.36.0 -o images.tar
```
...
```sh
docker run --rm -v ./config:/config morgankryze/cairn:1.19.3 -emit-icons > get-icons.sh
...
```sh
docker run --rm -v ./config:/config morgankryze/cairn:1.19.3 -emit-gatus > gatus-endpoints.yaml
```
...
docker run --rm -v ./config:/config:ro -v ./assets:/assets:ro \
  morgankryze/cairn:1.19.3 -check
```
...
docker push harbor.internal/cairn:1.19.3
...
```sh
helm push cairn-1.19.3.tgz oci://harbor.internal/helm
...
Container image "harbor.internal/cairn:1.19.3" already present on machine
...
helm install cairn oci://harbor.internal/helm/cairn --version 1.19.3 \
...
```sh
helm install cairn ./cairn-1.19.3.tgz -f values.yaml
```
...
```sh
helm upgrade cairn oci://harbor.internal/helm/cairn --version 1.19.3 -f values.yaml
```
