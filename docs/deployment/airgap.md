...
| -------------------- | -------- | -------------------------------------------------------------- |
| `cairn-1.17.0.tgz`   | 4 KB     | `helm pull`, the signed artifact rather than the folder in git |
...
```sh
helm pull oci://ghcr.io/morgankryze/charts/cairn --version 1.17.0

cosign verify ghcr.io/morgankryze/charts/cairn:1.17.0 \
  --certificate-identity-regexp '^https://github.com/MorganKryze/cairn/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```

The same two lines for `ghcr.io/morgankryze/cairn:1.17.0`, the image. Keep the
...
docker pull --platform $PLATFORM ghcr.io/morgankryze/cairn:1.17.0
...
```sh
docker tag ghcr.io/morgankryze/cairn:1.17.0 harbor.internal/cairn:1.17.0
docker tag ghcr.io/twin/gatus:v5.36.0 harbor.internal/gatus:5.36.0
docker save harbor.internal/cairn:1.17.0 harbor.internal/gatus:5.36.0 -o images.tar
```
...
```sh
docker run --rm -v ./config:/config ghcr.io/morgankryze/cairn:1.17.0 -emit-icons > get-icons.sh
...
```sh
docker run --rm -v ./config:/config ghcr.io/morgankryze/cairn:1.17.0 -emit-gatus > gatus-endpoints.yaml
```
...
docker run --rm -v ./config:/config:ro -v ./assets:/assets:ro \
  ghcr.io/morgankryze/cairn:1.17.0 -check
```
...
docker push harbor.internal/cairn:1.17.0
...
```sh
helm push cairn-1.17.0.tgz oci://harbor.internal/helm
...
Container image "harbor.internal/cairn:1.17.0" already present on machine
...
helm install cairn oci://harbor.internal/helm/cairn --version 1.17.0 \
...
```sh
helm install cairn ./cairn-1.17.0.tgz -f values.yaml
```
...
```sh
helm upgrade cairn oci://harbor.internal/helm/cairn --version 1.17.0 -f values.yaml
```
