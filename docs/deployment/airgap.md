...
| -------------------- | -------- | -------------------------------------------------------------- |
| `cairn-1.17.1.tgz`   | 4 KB     | `helm pull`, the signed artifact rather than the folder in git |
...
```sh
helm pull oci://ghcr.io/morgankryze/charts/cairn --version 1.17.1

cosign verify ghcr.io/morgankryze/charts/cairn:1.17.1 \
...
docker push harbor.internal/cairn:1.17.1
...
```sh
helm push cairn-1.17.1.tgz oci://harbor.internal/helm
...
Container image "harbor.internal/cairn:1.17.1" already present on machine
...
helm install cairn oci://harbor.internal/helm/cairn --version 1.17.1 \
...
```sh
helm install cairn ./cairn-1.17.1.tgz -f values.yaml
```
...
```sh
helm upgrade cairn oci://harbor.internal/helm/cairn --version 1.17.1 -f values.yaml
```
