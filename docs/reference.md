...
| `status.provider`     | `gatus`     | which monitor answers at `status.url`; `status.gatus` names its own, so a Gatus site needs neither key                                                                                                                    |
...
| `-hide-targets` | off       | with `-emit-gatus`: add the `ui` block that keeps each endpoint's [address off the Gatus dashboard](recipes/gatus.md#hiding-what-gatus-probes)                                                                                                                                                       |
