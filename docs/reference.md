...
| `desc`       | no       | empty         | text                                                                                                                                                                                                                                    |
| `details`    | no       | empty         | long text ([CommonMark, minus raw HTML](configuration/text.md)); enables the card's "Learn more" link                                                                                                                                   |
...
| `status.map`          | none        | `status.provider: json` only: `list`, `key`, `state`, then `up`, `degraded`, `maintenance`, the values that mean each ([mapping](recipes/status.md#any-other-status-api))                                                 |
...
| `-hide-targets` | off       | with `-emit-gatus`: add the `ui` block that keeps each endpoint's [address off the Gatus dashboard](recipes/status.md#hiding-what-gatus-probes)                                                                                                                                                      |
