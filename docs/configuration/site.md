...
| `status`       | none      | Live status pills fed by your monitor (`status.gatus`, or `status.url` with `status.provider` and, for Kuma, `status.slug`; then `status.page`, `status.interval`, `status.linked`, `status.insecure`, `status.ca`); see [Status page](../recipes/status.md). |
