...
| `search`        | `true`    | The box in the header. `search: false` removes it, the disabled copy every other page carries so the header keeps its shape, the script behind it and the `Cmd+K` shortcut. Worth it on a directory small enough to read at a glance, where the box is a row spent on nothing.                                   |
